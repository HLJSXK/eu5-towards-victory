...
- Follow-up fix: Diplomatic Alliance creation now has a single creation entry. The M1 grant no longer creates the IO; when the second Diplomatic Support pledge is accepted, `tv_seek_diplomatic_support` fires `tv_diplo_alliance.2`, and the event option calls `tv_create_diplomatic_alliance_effect`.
- Follow-up fix: `scripts/in_game/common/laws/gen_tv_alliance_laws.py` no longer emits `scope:recipient` inside generated IO policy `allow`, `on_activate`, or `on_deactivate`; those blocks read/mutate the current IO root directly. `scope:recipient` remains only in documented AI math contexts such as `wants_this_policy_bias` and `wants_propose_policy`.
