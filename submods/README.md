...
- Submods may depend on assets, localization, and scripts from their declared
  ...
  conflicting file.
- Each submod's `.metadata/thumbnail.png` is generated by
  `scripts/compat/gen_compat_thumbnails.py`: a diagonal top-left-to-bottom-right
  splice of `src_engineering_department/.metadata/thumbnail.png` (upper-right
  triangle) and the external mod's own thumbnail (lower-left triangle),
  matching the submods' own `hades.towards_victory.great_project`-family
  branding. Metadata must declare `"picture": "thumbnail.png"` for it to be
  picked up. Re-run after either source thumbnail changes.
