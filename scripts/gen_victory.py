...
HEADER = "# GENERATED by scripts/gen_victory.py — do not edit manually\n# Edit data/victory_paths.yaml and re-run gen_victory.py\n"
...
def gen_effects(data: dict) -> str:
    ...
    for p in paths:
        ...
        if p.get("score_var_init_in_checker"):
            ...
            lines.append(f"\tif = {{")
            lines.append(f"\t\tlimit = {{ NOT = {{ has_variable = {score_var} }} }}")
        ...
        lines.append(f"\tif = {{")
        lines.append(f"\t\tlimit = {{ NOT = {{ has_variable = tv_{pid}_milestone }} }}")
...
def gen_localization(data: dict, lang: str) -> str:
    ...
    shared = data["shared"]
    lines = [f"l_{'english' if lang == 'en' else 'simp_chinese'}:"]
