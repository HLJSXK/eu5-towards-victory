...
def generate() -> str:
    ...
    for event in events:
        ...
        lines.append(f' tv_engineering_department.{event_id}.a:0 "{q(option_loc(event["kind"], "a", "zh"))}"')
        if event["kind"] in {"trade_noneng_for_eng", "choose_eng_or_noneng_loss"}:
            lines.append(f' tv_engineering_department.{event_id}.b:0 "{q(option_loc(event["kind"], "b", "zh"))}"')
    return "\n".join(lines) + "\n"
