...
def render_separator(indent: str, *, visible: str | None = None) -> list[str]:
    ...
    lines.extend(
        [
            f"{indent}{T}size = {{ 100% {PANEL_SEPARATOR_HEIGHT} }}",
...
def render_slot_row(indent: str, *, slot: int) -> list[str]:
    ...
    lines.extend(
        [
            ...
            f"{indent}{T}{T}widget = {{",
            f"{indent}{T}{T}{T}size = {{ 50% 100% }}",
            f"{indent}{T}{T}{T}margin = {{ 0 4 }}",
            f"{indent}{T}{T}{T}using = bg_cabinet_card_frame",
        ]
    )
    lines.extend(render_slot_image_branches(indent + T * 3, slot=slot))
    lines.extend(
        [
            f"{indent}{T}{T}}}",
            f"{indent}{T}}}",
