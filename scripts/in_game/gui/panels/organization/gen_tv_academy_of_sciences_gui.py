...
def append_round_table(lines: list[str], level: int, visible_expr: str, table_text_key: str | None, history: bool = False) -> None:
    ...
    if not history:
        for x, y, crown in [
            ...
            emit(lines, level + 1, "widget = {")
            emit(lines, level + 2, f"position = {{ {x} {y} }}")
            emit(lines, level + 2, "size = { 34 34 }")
            emit(lines, level + 2, "using = bg_circle_piechart")
            emit(lines, level + 2, 'tooltip = "TV_ACADEMY_DEBATE_LOCAL_EMPTY_SEAT_TT"')
            if crown:
                emit(lines, level + 2, "text_single = {")
                emit(lines, level + 3, f'visible = "[{debate_active_expr()}]"')
                emit(lines, level + 3, "parentanchor = center")
                emit(lines, level + 3, "size = { 100% 100% }")
                emit(lines, level + 3, 'raw_text = "@crown_estate!"')
                emit(lines, level + 3, "fontsize = 18")
                emit(lines, level + 3, 'tooltip = "TV_ACADEMY_DEBATE_LOCAL_CROWN_SEAT_TT"')
