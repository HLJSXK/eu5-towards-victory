...
# The "engineering" IO leader (Great Engineer) belongs to the standalone
# Engineering Department mod. Its 3 actions are split into their own output
# there so that mod is self-contained; every other IO leader stays here.
ED_OUT_FILE = (
    REPO_ROOT / "src_engineering_department" / "in_game" / "common" / "generic_actions"
    / "tv_engineering_department_leader_actions.txt"
)
...
ED_IO_TYPE_SUBSTRINGS = ("engineering_department", "wonder")


def _is_ed_io(io: dict) -> bool:
    io_type = io.get("io_type", "").lower()
    return any(s in io_type for s in ED_IO_TYPE_SUBSTRINGS)
...
def _ai_creation_requirement_block(est: dict, indent: str) -> str:
...
def gen_build_headquarters(est: dict) -> str:
    ...
    ai_creation_requirement_allow = _ai_creation_requirement_block(est, "\t\t\t")
    ai_creation_requirement_enabled = _ai_creation_requirement_block(est, "\t\t\t\t")
    ai_creation_requirement_effect = _ai_creation_requirement_block(est, "\t\t\t\t\t")
    return (
        ...
        f"{ai_creation_requirement_allow}"
        f"\t\t}}\n"
        f"\t}}\n"
        f"\n"
        f"\tselect_trigger = {{\n"
        ...
        f"{ai_creation_requirement_enabled}"
        f"\t\t\t}}\n"
        ...
        f"{ai_creation_requirement_effect}"
        f"\t\t\t\t}}\n"
...
def generate(data: dict, establishment: dict | None = None, *, ed_only: bool = False) -> str:
    parts = [HEADER]
    if not ed_only and establishment and establishment.get("paths"):
...
def main() -> None:
    ...
    content = generate(data, establishment, ed_only=False)
    ed_content = generate(data, establishment, ed_only=True)
    ...
    if args.dry:
        ...
        ED_OUT_FILE.parent.mkdir(parents=True, exist_ok=True)
        ED_OUT_FILE.write_text(ed_content, encoding="utf-8-sig")
        print(f"Written: {ED_OUT_FILE.relative_to(REPO_ROOT)}")
