"""
Generate a bridge file that registers Towards Victory custom on_action names
under vanilla singleton pulses and other shared/hardcoded on_actions.

The generated file is a small additive bridge. It declares only parent hook keys
plus TV-owned `on_actions = { ... }`, letting vanilla or feature files provide
the events/random_events/effect bodies. This avoids copying vanilla pulse
content into the mod, avoids vanilla filename collisions, and avoids scattering
singleton bridge blocks across feature files.

Usage:
    conda run --no-capture-output -n eu5 python scripts/in_game/common/on_action/gen_tv_pulse_registry.py
"""

import sys
import yaml
from pathlib import Path

if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")

REPO_ROOT = Path(__file__).resolve().parents[4]

REGISTRY = REPO_ROOT / "data" / "pulse_registry.yaml"

HARDCODED_ON_ACTIONS = REPO_ROOT / "reference_game_files" / "game" / "in_game" / "common" / "on_action" / "_hardcoded.txt"

SOURCE_FILES: dict[str, Path | None] = {
    "monthly_country_pulse": REPO_ROOT / "reference_game_files" / "game" / "in_game" / "common" / "on_action" / "country_monthly.txt",
    "yearly_country_pulse":  REPO_ROOT / "reference_game_files" / "game" / "in_game" / "common" / "on_action" / "country_yearly.txt",
    "on_character_death":    REPO_ROOT / "reference_game_files" / "game" / "in_game" / "common" / "on_action" / "character_death_pulses.txt",
    "on_ruler_death":        HARDCODED_ON_ACTIONS,
    "on_game_start":         HARDCODED_ON_ACTIONS,
    # on_game_load is fired by the CMF scripted GUI layer, not vanilla
    # _hardcoded.txt. Both deployable TV mods therefore declare CMF as a hard
    # metadata dependency.
    "on_game_load":          None,
    "on_work_of_art_created": HARDCODED_ON_ACTIONS,
    "on_work_of_art_destroyed": HARDCODED_ON_ACTIONS,
    "on_work_of_art_looted": HARDCODED_ON_ACTIONS,
    "on_location_occupied": HARDCODED_ON_ACTIONS,
    "on_siege_won":          HARDCODED_ON_ACTIONS,
    "on_loan_repaid":        HARDCODED_ON_ACTIONS,
    "on_loan_renewed":       HARDCODED_ON_ACTIONS,
    "on_bankruptcy":         HARDCODED_ON_ACTIONS,
    "on_exploration_success": REPO_ROOT / "reference_game_files" / "game" / "in_game" / "common" / "on_action" / "exploration_mission_monthly.txt",
    "on_colonial_charter_finished": HARDCODED_ON_ACTIONS,
    "on_colonial_charter_failed": HARDCODED_ON_ACTIONS,
    "on_took_location_in_peace_treaty": HARDCODED_ON_ACTIONS,
    "on_reform_change":      HARDCODED_ON_ACTIONS,
    "on_bureaucracy_added":  HARDCODED_ON_ACTIONS,
    "on_religion_changed":   HARDCODED_ON_ACTIONS,
    "on_cabinet_assigned":   HARDCODED_ON_ACTIONS,
    "on_cabinet_removed":    HARDCODED_ON_ACTIONS,
    "on_parliament_passed":  HARDCODED_ON_ACTIONS,
    "on_policy_changed":     HARDCODED_ON_ACTIONS,
    "on_character_moved_country": HARDCODED_ON_ACTIONS,
    "on_new_ruler":          HARDCODED_ON_ACTIONS,
    "on_character_marriage": HARDCODED_ON_ACTIONS,
    "on_royal_marriage":     HARDCODED_ON_ACTIONS,
    "on_dependency_gained":  HARDCODED_ON_ACTIONS,
    "on_becoming_free":      HARDCODED_ON_ACTIONS,
    "on_subject_created":    HARDCODED_ON_ACTIONS,
    "on_transfer_subject":   HARDCODED_ON_ACTIONS,
    "on_war_declared":       HARDCODED_ON_ACTIONS,
    "on_join_war":           HARDCODED_ON_ACTIONS,
    "on_ending_war":         HARDCODED_ON_ACTIONS,
    "on_pre_winning_war":    HARDCODED_ON_ACTIONS,
    "on_winning_war":        HARDCODED_ON_ACTIONS,
    "on_losing_war":         HARDCODED_ON_ACTIONS,
    "on_location_changed_owner": HARDCODED_ON_ACTIONS,
    "on_location_changed_rank": HARDCODED_ON_ACTIONS,
    "on_institution_embraced": HARDCODED_ON_ACTIONS,
    "on_international_organization_changed_leader": HARDCODED_ON_ACTIONS,
    "on_capital_moved":      HARDCODED_ON_ACTIONS,
    # CMF scripted GUIs fire this custom callback hook.
    "cmf_on_callback":       None,
}

OUTPUT_FILE = REPO_ROOT / "src" / "in_game" / "common" / "on_action" / "tv_pulse_bridges.txt"
# The standalone Engineering Department mod registers its own on_action ids
# (wonder/engineering_department-prefixed) directly onto the same vanilla
# hooks, in its own file — on_actions lists merge additively across files
# (confirmed: vanilla's ai_personalities_setup.txt and this mod's own
# tv_pulse_bridges.txt already both extend on_game_start independently).
ED_OUTPUT_FILE = (
    REPO_ROOT / "src_engineering_department" / "in_game" / "common" / "on_action"
    / "tv_engineering_department_pulse_bridges.txt"
)
COURT_OUTPUT_FILE = (
    REPO_ROOT / "src_court_positions" / "in_game" / "common" / "on_action"
    / "tv_court_positions_pulse_bridges.txt"
)

SCRIPT_REL = "scripts/in_game/common/on_action/gen_tv_pulse_registry.py"
DATA_REL    = "data/pulse_registry.yaml"

ED_ON_ACTION_SUBSTRINGS = ("engineering_department", "wonder")
COURT_ON_ACTION_SUBSTRINGS = ("court_positions",)


def _is_ed_on_action(name: str) -> bool:
    lname = name.lower()
    return any(s in lname for s in ED_ON_ACTION_SUBSTRINGS)


def _is_court_on_action(name: str) -> bool:
    return any(s in name.lower() for s in COURT_ON_ACTION_SUBSTRINGS)

HEADER_TEMPLATE = """\
# @Generated by {script}
#   Data:    {data}
#   Regen:   conda run --no-capture-output -n eu5 python {script}
# Do not edit directly — modify the data file and re-run the generator.

"""

def extract_on_action_block(lines: list[str], hook_name: str) -> list[str]:
    """Return only the named on_action block from a multi-action source file."""
    start_idx: int | None = None
    depth = 0

    for i, raw in enumerate(lines):
        stripped = raw.strip()
        if start_idx is None:
            if stripped.startswith(hook_name) and "=" in stripped and "{" in stripped:
                start_idx = i
                depth = stripped.count("{") - stripped.count("}")
                if depth <= 0:
                    return [raw]
            continue

        depth += stripped.count("{") - stripped.count("}")
        if depth <= 0:
            return lines[start_idx:i + 1]

    raise ValueError(f"on_action block not found or unterminated: '{hook_name}'")


def build_bridge_block(hook_name: str, additions: list[str]) -> list[str]:
    """Build a small bridge that registers TV-owned on_actions."""
    lines = [
        f"{hook_name} = {{\n",
        "\ton_actions = {\n",
        "\t\t# Towards Victory additions\n",
    ]
    for name in additions:
        lines.append(f"\t\t{name}\n")
    lines.extend([
        "\t}\n",
        "}\n",
    ])
    return lines


def verify_source(hook_name: str) -> None:
    src_path = SOURCE_FILES[hook_name]

    if src_path is None:
        print(f"Skipping source verification for custom hook: {hook_name}")
    else:
        print(f"Reading source: {src_path.relative_to(REPO_ROOT)}")
        # Use utf-8-sig to strip UTF-8 BOM if present (some vanilla files start with BOM)
        text = src_path.read_text(encoding="utf-8-sig")
        lines = text.splitlines(keepends=True)
        extract_on_action_block(lines, hook_name)


def main() -> None:
    registry = yaml.safe_load(REGISTRY.read_text(encoding="utf-8"))
    groups = [
        ("singleton pulse/death bridges", registry.get("pulses", {})),
        ("general/hardcoded hook bridges", registry.get("bridges", {})),
    ]

    blocks: list[str] = []
    ed_blocks: list[str] = []
    court_blocks: list[str] = []
    for label, hooks in groups:
        group_blocks: list[str] = []
        ed_group_blocks: list[str] = []
        court_group_blocks: list[str] = []
        for hook_name, additions in hooks.items():
            if hook_name not in SOURCE_FILES:
                print(f"WARNING: no source mapping for '{hook_name}', skipping")
                continue
            verify_source(hook_name)
            main_additions = [a for a in additions if not _is_ed_on_action(a) and not _is_court_on_action(a)]
            ed_additions = [a for a in additions if _is_ed_on_action(a)]
            court_additions = [a for a in additions if _is_court_on_action(a)]
            if main_additions:
                group_blocks.append("".join(build_bridge_block(hook_name, main_additions)).rstrip())
            if ed_additions:
                ed_group_blocks.append("".join(build_bridge_block(hook_name, ed_additions)).rstrip())
            if court_additions:
                court_group_blocks.append("".join(build_bridge_block(hook_name, court_additions)).rstrip())
        if group_blocks:
            blocks.append(f"# {label}")
            blocks.extend(group_blocks)
        if ed_group_blocks:
            ed_blocks.append(f"# {label}")
            ed_blocks.extend(ed_group_blocks)
        if court_group_blocks:
            court_blocks.append(f"# {label}")
            court_blocks.extend(court_group_blocks)

    header = HEADER_TEMPLATE.format(script=SCRIPT_REL, data=DATA_REL)
    output = header + "\n\n".join(blocks) + "\n"
    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_FILE.write_text(output, encoding="utf-8-sig")
    print(f"Wrote: {OUTPUT_FILE.relative_to(REPO_ROOT)}")

    ed_output = header + "\n\n".join(ed_blocks) + "\n"
    ED_OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    ED_OUTPUT_FILE.write_text(ed_output, encoding="utf-8-sig")
    print(f"Wrote: {ED_OUTPUT_FILE.relative_to(REPO_ROOT)}")

    court_output = header + "\n\n".join(court_blocks) + "\n"
    COURT_OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    COURT_OUTPUT_FILE.write_text(court_output, encoding="utf-8-sig")
    print(f"Wrote: {COURT_OUTPUT_FILE.relative_to(REPO_ROOT)}")

    print("Done.")


if __name__ == "__main__":
    main()
