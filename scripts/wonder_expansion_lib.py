...
NEW_WONDER_MAX_ID = UNIQUE_WONDER_MAX_ID
...
def load_unique_wonders() -> list[dict]:
    ...
    for raw in unique_data.get("unique_wonders", []):
        ...
        if base_key not in expansion["buildings"]:
            raise ValueError(f"Missing building data for base wonder {base_key}")
        if base_key not in expansion["base_modifiers"]:
...
def load_all_wonder_mechanics(*, include_unique: bool = True) -> tuple[list[dict], dict]:
    wonders, expansion = load_generic_wonder_mechanics()
    if include_unique:
...
def load_new_wonder_data() -> tuple[list[dict], dict]:
    wonders, expansion = load_expansion_wonder_data()
    wonders.extend(load_unique_wonders())
    return wonders, expansion


def render_header(script_rel: str, data_rel: str = "data/wonders.yaml + data/wonder_expansion.yaml + data/unique_wonders.yaml") -> list[str]:
    return [
