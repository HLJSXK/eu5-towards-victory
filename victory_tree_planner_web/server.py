...
from .service import GENERATED_PREVIEWS_DIR, TREE_PREVIEW_URL_PREFIX, VictoryTreePlannerService

PACKAGE_ROOT = Path(__file__).resolve().parent
STATIC_DIR = PACKAGE_ROOT / "static"
service = VictoryTreePlannerService()
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="Victory Tree Node Planner Web",
        docs_url=None,
        redoc_url=None,
    )
    ...
    @app.post("/api/save")
    def save(request: SavePositionsRequest) -> dict:
        try:
            return service.save_positions(request.edits)
