...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the web-based Victory Tree node position planner.")
    parser.add_argument(
        "--check",
        action="store_true",
        help="load and validate the node position data, then exit without starting the web app",
    )
    parser.add_argument("--host", default="127.0.0.1", help="host interface for the web server")
    parser.add_argument("--port", type=int, default=8767, help="port for the web server")
    ...
    print(f"Victory Tree Node Planner Web running at {url}")
    uvicorn.run("victory_tree_planner_web.server:app", host=args.host, port=args.port, reload=args.reload)
