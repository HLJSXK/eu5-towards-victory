...
from wonder_mechanics.rituals import STYLE_3_REWARD_EFFECTS, load_cost_reward_unit_entries

COST_REWARD_CATALOGS = (
    "country_reward",
    "local_reward",
    "character_reward",
    "country_modifier",
...
def scan_cost_reward_unit_types() -> dict[str, list[str]]:
    catalogs = load_cost_reward_unit_entries()
    return {
        catalog: sorted(
            str(entry["id"])
        ...
        for catalog in COST_REWARD_CATALOGS
    }
...
def build_catalog() -> dict[str, object]:
    ...
    reference_modifier_types = scan_event_static_modifier_effects(event_static_modifier_names, modifier_type_categories)
    event_effects = scan_event_effects()
    reward_types, reward_sources = scan_style_3_reward_types(event_effects)
    cost_reward_unit_types = scan_cost_reward_unit_types()
    modifier_types = {
        "country": sorted(set(reference_modifier_types["country"]) | set(cost_reward_unit_types["country_modifier"])),
        "local": sorted(set(reference_modifier_types["local"]) | set(cost_reward_unit_types["local_modifier"])),
    }
    return {
        "modifier_types": modifier_types,
        "cost_reward_unit_types": cost_reward_unit_types,
        ...
        "scan_summary": {
            ...
            "reference_country_modifier_count": len(reference_modifier_types["country"]),
            "reference_local_modifier_count": len(reference_modifier_types["local"]),
            "cost_reward_country_modifier_count": len(cost_reward_unit_types["country_modifier"]),
            "cost_reward_local_modifier_count": len(cost_reward_unit_types["local_modifier"]),
            ...
            "cost_reward_unit_count": sum(len(entries) for entries in cost_reward_unit_types.values()),
        },
    }
...
def main() -> None:
    ...
    header = (
        ...
        "#   Data:    data/cost_reward_units.yaml + reference_game_files/game/in_game/events + reference_game_files/game/main_menu/common/static_modifiers + reference_game_files/game/main_menu/common/modifier_type_definitions\n"
        "#   Regen:   C:\\Users\\Hades\\anaconda3\\envs\\eu5\\python.exe scripts_engineering_department\\gen_wonder_editor_catalog.py\n"
    ...
    print(
        ...
        f"{summary['cost_reward_unit_count']} cost/reward units)"
    )
