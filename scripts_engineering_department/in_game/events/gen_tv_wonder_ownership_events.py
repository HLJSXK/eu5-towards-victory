...
OUT_FILE = REPO_ROOT / "src_engineering_department" / "in_game" / "events" / "tv_wonder_ownership_events.txt"
SCRIPT_REL = "scripts_engineering_department/in_game/events/gen_tv_wonder_ownership_events.py"
T = "\t"
...
def append_ownership_loss_event(lines: list[str], wonder: dict, mechanics: dict) -> None:
    ...
    lines.append(f'{T}image = "{wonder_event_image(wonder)}"')
    lines.append(f"{T}outcome = negative")
    lines.append("")
