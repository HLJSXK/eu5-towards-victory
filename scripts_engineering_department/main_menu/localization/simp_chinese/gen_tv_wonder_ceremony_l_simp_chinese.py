...
def generate() -> str:
    ...
    for stage in range(1, STAGE_COUNT + 1):
        ...
        lines.append(f' {pay_option_tooltip_key(stage)}:0 "#G 仪式进度将推进#!"')
        lines.append(f' {decline_option_tooltip_key(stage)}:0 "#R 仪式进度将推迟三个月#!"')
    ...
    for wonder in wonders:
        ...
        for stage_index, stage_data in enumerate(stages, start=1):
            lines.append(f' {card_icon_key(stage_index, wonder["id"])}:0 "@{ceremony_icon_alias(stage_data["icon"])}!"')
            lines.append(
