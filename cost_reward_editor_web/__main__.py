...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the web-based Cost/Reward & Modifier unit editor.")
    parser.add_argument(
        "--check",
        action="store_true",
        help="load and validate the unit catalog, then exit without starting the web app",
    )
    parser.add_argument("--host", default="127.0.0.1", help="host interface for the web server")
    parser.add_argument("--port", type=int, default=8766, help="port for the web server")
    ...
    print(f"Cost/Reward Token Editor Web running at {url}")
    uvicorn.run("cost_reward_editor_web.server:app", host=args.host, port=args.port, reload=args.reload)
