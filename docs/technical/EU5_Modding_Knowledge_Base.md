...
`scope:vote.modifier_utility(scope:actor)` is displayed as `POLICY_MODIFIER_UTILITY` and multiplied in `policy_vote`. For IO laws whose `country_modifier` is intended only for the leader country or another subset of members, put that filter inside the modifier block with `potential_trigger`, for example `country_modifier = { potential_trigger = { is_leader_of_international_organization = scope:recipient } ... }`. Otherwise non-target members, especially subject-chain members, can treat the unfiltered policy modifier as their own direct benefit and become extremely biased toward a policy.
