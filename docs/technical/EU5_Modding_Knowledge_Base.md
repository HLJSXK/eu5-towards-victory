...
Dynamic game-concept links are unsafe when the dynamic value can be localized before link parsing. In a location-window test, `SelectGameConcept(dynamic_key, ...)` rendered the localized wonder name and the parser then tried to evaluate `[圣山|e]`, logging `Could not find data system function '圣山'`. For dynamic names, display plain text with `Localize(Concatenate('game_concept_', key))`. For clickable concept links, generate static branches with literal `[concept|E]` text.
