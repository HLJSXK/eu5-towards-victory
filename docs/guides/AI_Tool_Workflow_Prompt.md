...
| 2026-05 | Reused event/custom-tooltip localization containing `[tv_arts_exhibition_mechanism\|e]` inside a GUI `TooltipTextBlock` | Assumed game-concept loc markup is accepted in all localized text contexts | GUI text blocks parse square brackets as data-system expressions; use a GUI-safe key without `[concept\|e]` markup for GUI previews and keep concept links in event/custom_tooltip text. Error: `Could not find data system function 'tv_arts_exhibition_mechanism'`. |
