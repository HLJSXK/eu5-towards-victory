...
|---|---|---|---|
| 2026-05 | Used `tv_academy_member` as both the ordinary voter group and the proposer for a one-country Academy IO parliament | Assumed the founding country counted as an ordinary member proposal group | For one-country or founder-led IO parliaments, add a separate leader special status with `leader = yes`, voting power, participation modifiers, and attach leader-proposed issues/agendas to that status, mirroring HRE `emperor` issues. |
