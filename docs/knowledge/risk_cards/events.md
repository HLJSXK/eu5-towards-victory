...
5. Let visible building effects render their own costs.
   If an event option starts real building construction, put the `construct_building`
   or `change_building_level_in_location` effect directly in the visible option chain
   whenever its tooltip is useful. Do not write option text such as "Spend 400 ducats"
   and then hide `pay_price` plus free/instant construction in a hidden helper. The engine
   renders the building name, price, modifiers, and cost modifiers from the visible
   building effect. Reserve manual `pay_price` / `cost_multiplier = 0` only for genuinely
   event-funded, nonstandard, or intentionally hidden construction flows.

6. Do not wrap visible option effects in `effect = { ... }`.
...
7. Hide application chains when nested tooltips are not needed.
...
8. Do not treat option `hidden_effect` as a performance boundary.
