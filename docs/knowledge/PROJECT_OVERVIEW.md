...
The **Scientific Victory** path (科技胜利) is fully implemented with a unique building gate and advance-locking system: 5 triggers, 5 reward modifiers, 5 events, and localization in English and Simplified Chinese. Score = `num_of_advances_researched` (snapshot taken monthly by tv_update_science_score_effect); thresholds at 100 / 150 / 200 / 300 / 400 (provisional, calibrate after playtesting). Rewards: M1 research speed, M2 institution spread/discipline, M3 institution spread/pop growth, M4 institution embrace cost/production, M5 production/discipline. **Unique mechanic:** Each milestone only fires after the corresponding **Academy of Sciences** building phase is manually constructed in the capital. The building chain (Phases I–V) is visible from game start; each phase requires the previous phase plus the corresponding score threshold (`tv_academy_of_sciences_level` variable). Construction conditions use `custom_tooltip` (score variable is not auto-parseable). **Academy of Sciences IO** (`tv_academy_of_sciences`): non-unique IO created when Phase I building (`tv_academy_of_sciences_1`) is built — fires event `tv_academy.1` which sets country variable `tv_academy_io_member = 1` and calls `create_international_organization`. IO uses `leader_type = character` with `leader_change_trigger_type = rulerchange`; the IO leader is always the current ruler of the leader country (Chief Scientist). **Research Mechanism (Phases 2–5):** Four sub-processes unlock a chosen target advance. Sub-process A (Interest Collection): monthly random event (`tv_research.1`, ~20% base chance) adds +1 to `tv_research_interest`; completes at 5. Sub-process B (Prerequisites, Phase 4 stub = `always`): auto-completes on first monthly tick in phase 1. Sub-process C (Literacy Foundation): **IO variable** `tv_research_c_progress` accumulates `average_country_literacy × 0.05` per month; completes at 100 (~40 months at 50% literacy). Sub-process D (Concentrated Research): **IO variable** `tv_research_d_progress` accumulates `ruler_or_heir_if_regent.adm × 1.0` per month; completes at 100 (~17 months at adm=6). Monthly gold cost during D: `(ruler.dip + ruler.mil) × −0.5`. **Target selection:** Player clicks "Request Research Topic" → semi-random proposal from still-locked advances → event `tv_research.10` for confirmation; abandoned targets set 12-month cooldown. **Phase 5:** 10 advances are locked at game start (variables unset → allow blocks return false): 1=printing_press_advance, 2=iron_working, 3=anatomy_advance, 4=gunpowder_advance, 5=blast_furnace, 6=scientific_experimentation, 7=scientific_revolution_advance, 8=manufactories_advance, 9=industrialization_advance, 10=vaccination_advance. Advance allow-block overrides in `tv_locked_advances.txt` (REPLACE for advances with institution-based allow; INJECT for others). The OR logic preserves vanilla institution path. Generated by `data/locked_advances.yaml` → `gen_locked_advances.py`. **Cycle phases:** 0=idle, 1=A/B/C running, 2=D concentrated research. State variables: `tv_research_phase`, `tv_research_target`, `tv_research_interest`, `tv_research_b_done`, `tv_research_selection_cd`; display vars: `tv_rm_subprocess_a/b/c`, `tv_rm_subprocess_d_ready`. IO panel (`tv_academy_of_sciences.gui`) shows target, CD, Chief Scientist name, subprocess A/B/C/D status, and three action buttons. Victory situation panel Section 2 Academy sub-block shows the same subprocess status and target advance name.
...
7. **Academy of Sciences Building Chain** — Five capital buildings (`tv_academy_of_sciences_1`–`5`) gate Scientific Victory milestones. Each requires the previous phase built and the corresponding score threshold; `on_built` immediately triggers `tv_check_science_milestones_effect`. Building category: `cultural_category`; visible in all capitals from game start. Phase I `on_built` also fires event `tv_academy.1` which creates the **Academy of Sciences IO** (`tv_academy_of_sciences`), a non-unique character-leader IO. Membership flag: `tv_academy_io_member` (country variable set to 1 on join). IO leader (Chief Scientist) = leader country's current ruler; updates automatically on ruler change via `leader_change_trigger_type = rulerchange`.
8. **Research Mechanism** — Full four-subprocess cycle unlocking locked advances. Sub-process A: monthly random interest events (5 needed; event namespace `tv_research`). Sub-process B: passive prerequisite check (Phase 4 stub = `always`). Sub-process C: IO variable `tv_research_c_progress` auto-accumulates via `monthly_change`; completed by `monthly_effect`. Sub-process D: IO variable `tv_research_d_progress` auto-accumulates; costs gold monthly; triggers unlock on completion. Three generic IO-panel actions: `tv_request_research_target`, `tv_abandon_research_target`, `tv_start_concentrated_research`. Lifecycle managed by scripted effects in `tv_research_subprocess_effects.txt`; conditions in `tv_research_subprocess_triggers.txt`. **Phase 5 targets (10 advances):** 1=printing_press, 2=iron_working, 3=anatomy, 4=gunpowder, 5=blast_furnace, 6=scientific_experimentation, 7=scientific_revolution, 8=manufactories, 9=industrialization, 10=vaccination. Each is listed in `data/locked_advances.yaml`; triggers/effects/loc generated by `scripts/gen_locked_advances.py`. Advance allow-block overrides live in `src/in_game/common/advances/tv_locked_advances.txt` — uses REPLACE for advances with existing allow blocks (institution-based), INJECT for advances with none. OR logic in allow preserves the original institution-based research path alongside the Academy unlock path.
...
├── in_game/
│   ├── common/
│   │   ├── advances/                      tv_locked_advances.txt  [MANUAL — REPLACE/INJECT allow-block gates for 10 locked advances]
│   │   ├── situations/                    towards_victory_situations.txt  [GENERATED]
│   │   ├── scripted_triggers/             towards_victory_triggers.txt  [GENERATED]
│   │   │                                  tv_advance_lock_triggers.txt  [GENERATED — tv_advance_<id>_is_locked triggers]
│   │   │                                  tv_research_subprocess_triggers.txt  [MANUAL — Research Mechanism conditions]
│   │   ├── scripted_effects/              towards_victory_effects.txt  [GENERATED]
│   │   │                                  towards_victory_leaderboard.txt  [MANUAL — leaderboard effects]
│   │   │                                  tv_advance_unlock_effects.txt  [GENERATED — tv_unlock_advance_<id>_effect]
│   │   │                                  tv_research_subprocess_effects.txt  [MANUAL — Research Mechanism lifecycle effects]
...
│   │   │                                  tv_academy_of_sciences.txt  [MANUAL — Research Mechanism IO; character leader; C/D IO variables]
...
│                                          tv_academy_of_sciences.gui  [MANUAL — Research Mechanism IO panel with action buttons]
